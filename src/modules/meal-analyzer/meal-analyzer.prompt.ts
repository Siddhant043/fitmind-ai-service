...
} from './meal-analyzer.cuisine-guidelines.js'
...
function formatDietaryPrefLabel(dietaryPref: string | null): string {
  if (!dietaryPref) return 'not set'
  ...
  return labels[dietaryPref] ?? dietaryPref
}

function buildUserContextBlock(userContext: MealAnalyzerUserContext): string {
  const countryDisplay = formatCountryDisplayName(userContext.countryCode)
  const countryLine =
    userContext.countryCode && countryDisplay
      ? `- Country: ${countryDisplay} (${userContext.countryCode})`
      : '- Country: not set'

  return `
User profile for personalised feedback:
${countryLine}
- Dietary preference: ${formatDietaryPrefLabel(userContext.dietaryPref)}
...
No user profile context is available. Write a generic but encouraging mealFeedback based on the meal macros alone. Set workoutSuggestion to null.`
  ...
  const persona = buildCuisinePersona(userContext?.countryCode ?? null)
  const cuisineGuidelines = buildCuisineGuidelines(userContext?.countryCode ?? null)
...
- Write mealFeedback: a 1-2 sentence personalised insight comparing this meal's macros to the user's daily targets and fitness goal. Be specific with numbers and percentages when targets are available. Respect the user's country and dietary preference when suggesting food-specific improvements.
